...
### 方法一：最简单的方式 (Easiest Way)
```bash
# 1. 克隆项目
git clone https://github.com/xiaolinbaba/Madopic.git
cd Madopic

# 2. 自动设置依赖 (推荐 - 会下载本地依赖，提高加载速度)
npm run setup
# 或者直接运行: ./setup.sh
...
## 🔧 本地部署 (Alternative Methods)

如果你没有 Node.js，也可以使用以下方法：
...
### 应用加载很慢？
**问题**：页面加载时间很长，显示 "应用出现错误"

**解决方案**：
1. 运行 `npm run setup` 下载本地依赖
2. 或者运行 `./setup.sh`
3. 刷新页面

**原因**：默认情况下应用从 CDN 加载依赖，有时会超时。
...
2. 或者使用 Python 方法启动服务器

### 数学公式不显示？
确保网络连接正常，或运行 `npm run setup` 下载本地依赖。
